...
async function saveICSToPublic(icsContent: string): Promise<string> {
    const publicDir = path.join(process.cwd(), 'public');
    const filePath = path.join(publicDir, 'calendar.ics');
    ...
    // Escrever o arquivo (funciona localmente, mas no Vercel o public é readonly)
    // No Vercel, usamos o cache em memória em vez de escrever no filesystem
    try {
        fs.writeFileSync(filePath, icsContent, 'utf-8');
    } catch (error) {
        // Em produção no Vercel, o public é readonly, então apenas logamos
        console.log('Note: Cannot write to public directory in production (Vercel limitation). Using in-memory cache instead.');
    }
...
export async function GET(request: NextRequest) {
    try {
        ...
        // Salvar no diretório public (localmente) ou cache (produção)
        const filePath = await saveICSToPublic(icsContent);
        
        // Atualizar cache compartilhado (usado pela rota /calendar.ics)
        setICSCache(icsContent);
        
        console.log(`Calendar file updated with ${events.length} events`);
        
        return new Response(JSON.stringify({
            success: true,
            message: 'Calendar file updated successfully',
            eventsCount: events.length,
            filePath: '/calendar.ics',
            note: 'In production (Vercel), the file is served via /calendar.ics route with in-memory cache'
        }), {
            headers: {
        ...
        console.error('Error updating calendar file:', error);
        return new Response(JSON.stringify({
            success: false,
